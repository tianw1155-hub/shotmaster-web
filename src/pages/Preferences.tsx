...
export function PreferencesPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-surface via-surface to-primary/10">
      {/* 游客提示 */}
      {user.isGuest && (
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
            ...
            <button
              ...
              className={`relative p-5 rounded-2xl border-2 transition-all duration-200 text-left ${
                isSelected
                  ...
                  : 'border-gray-200 bg-surface hover:border-gray-300 hover:shadow-md'
              }`}
            >
              {/* 选中标记 */}
              {isSelected && (
        ...
        <div className="space-y-3">
          <button
            ...
            className="w-full py-4 rounded-2xl bg-gradient-to-r from-primary to-accent text-white font-semibold shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {isLoading ? (
              <>
          ...
          <p className="text-center text-ink-light text-sm">
            已选择 <span className="text-primary font-medium">{selected.length}</span> 项
          </p>
        </div>
      </div>
    </div>
