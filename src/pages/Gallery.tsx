  ...
  const planSections: { key: ShootingPlanDimension; icon: React.ElementType; title: string; color: string; content: (plan: ShootingPlan) => React.ReactNode }[] = [
      ...
      content: (p) => (
        <div className="space-y-1 text-sm">
          <p className="text-ink-secondary">
            方向：<span className="text-ink font-medium">{p.lighting.direction}</span>
          </p>
          <p className="text-ink-secondary">
            质量：<span className="text-ink font-medium">{p.lighting.quality}</span>
          </p>
          <p className="text-ink-secondary">
            色温：<span className="text-ink font-medium">{p.lighting.colorTemp}</span>
          </p>
    ...
    },
    {
      ...
      content: (p) => (
        <div className="space-y-1 text-sm">
          <p className="text-ink-secondary">
            相机：<span className="text-ink font-medium">{p.equipment.camera}</span>
          </p>
          <p className="text-ink-secondary">
            镜头：<span className="text-ink font-medium">{p.equipment.lens}</span>
          </p>
          <div className="flex flex-wrap gap-1 mt-2">
            {p.equipment.accessories.map((a, i) => (
              <span key={i} className="px-2 py-1 rounded-sm bg-ink-muted/5 text-ink-secondary text-xs">
                {a}
              </span>
            ))}
          </div>
              ...
              return (
                <div key={key} className="border border-line rounded-md overflow-hidden">
                  <button
                    ...
                    className="w-full p-3 flex items-center justify-between hover:bg-surface-muted/60 transition-colors"
                  >
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        title="这部分建议有用"
                      >
                        <ThumbsUp className={`w-3.5 h-3.5 ${dimFeedback.liked ? 'fill-current' : ''}`} strokeWidth={1.25} />
                      </button>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        title="这部分建议没用"
                      >
                        <ThumbsDown className={`w-3.5 h-3.5 ${dimFeedback.disliked ? 'fill-current' : ''}`} strokeWidth={1.25} />
                      </button>
                      {isExpanded ? (
                  ...
                  {isExpanded && (
                    ...
                    >
                      <div className="pt-3">{content(shootingPlan)}</div>
                    </motion.div>
                  )}
                </div>
              );
