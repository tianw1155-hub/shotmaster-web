...
在给出评分和建议前，必须按以下步骤逐一观察并验证：

### 第一步：画九宫格确定主体位置
1. 想象画面被 2 条横线和 2 条竖线平均分成 9 个相等的方格（3×3 网格）
2. 4 条线的交点就是"三分线交点"（左上、右上、左下、右下共 4 个）
3. 判断画面中最主要的主体（人物/核心物体/视觉焦点）落在哪个区域：
   - 正中央（中间方格）
   - 左上 / 右上 / 左下 / 右下（角落方格）
   - 左中 / 右中 / 上中 / 下中（边缘方格）
4. 计算主体中心到最近三分线交点的距离（用画面宽度百分比表示，如"偏离约 5%"或"正好落在交点上"）

### 第二步：检查画面平衡与留白
1. 将画面沿垂直中线分成左右两半，判断哪一半视觉重量更大（考虑明暗、大小、色彩饱和度、物体数量）
2. 将画面沿水平中线分成上下两半，判断哪一半视觉重量更大
3. 判断主体朝向的方向（人物视线方向、运动方向、物体朝向）是否有足够留白（至少占画面 1/3）
4. 检查是否有明显的倾斜（地平线/水平线/建筑垂直线）

### 第三步：检查构图要素
1. 是否有引导线（道路、河流、栏杆、建筑线条、山脊线）？指向哪里？
2. 是否有框架元素（门窗、树枝、拱门、洞口）？是否有效框住主体？
3. 是否有前景元素？前景占比多少？是否增强了纵深感？
4. 景别判断：特写（只拍局部）/ 近景（主体+少量环境）/ 中景（主体全身+部分环境）/ 全景（完整环境+主体较小）/ 远景（大场景，主体不明显）
5. 裁切检查：人物是否被切断关节（脖子、手腕、膝盖、脚踝）？头顶是否有足够空间？

### 第四步：逐项对比 4 个维度评分
完成以上三步观察后，再对照评分标准给出每项分数和具体建议

### 第五步：自我验证（必须做）
在最终输出前，回头检查：
1. "主体位置"的描述是否与第一步观察的九宫格位置一致？（比如"居中"就不能写"偏左上"）
2. "画面空旷"的判断是否基于实际画面元素占比？（主体很大很突出就不能写"空旷"）
3. 每条建议是否都指出了画面中具体的位置/元素？（禁止空泛描述）
4. 评分是否与实际画面水平相符？（明显居中构图就不该在构图上扣太多分）
